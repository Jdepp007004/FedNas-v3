...
The default workspace is created automatically when the host starts. Neither
the host nor clients enter a project ID. The client page selects the first open
...
4. No project ID or account credentials
...
On each participant laptop, install the client dependencies once and then run:

```powershell
python client_app.py
```

The client page opens automatically. Enter the participant name and host ngrok
link, choose the local `client_X.csv` split, and enter the resources to
dedicate. The project is selected automatically. After the host approves the
name, the same script starts the native worker automatically; no project ID,
account, password, or second command is needed.

**All CLI flags:**

| Flag | Required | Description |
|------|----------|-------------|
| `python client_app.py` | ✅ | Opens the client setup page and native worker |
...
1. Open **http://localhost:8000/host**; it opens automatically when `host_app.py` starts.
2. Under **Participants**, find the pending display name.
3. Click **Approve**.
4. The participant automatically moves to **Connected** and its worker begins training.
